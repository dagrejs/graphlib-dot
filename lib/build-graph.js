...
function handleEdgeStmt(g, stmt, defaultStack, sg) {
  ...
  _.each(stmt.elems, function(elem) {
    ...
    switch(elem.type) {
      case "node": curr = [elem]; break;
      case "subgraph": curr = collectNodes(elem); break;
    }

    _.each(prev, function(v) {
      _.each(curr, function(w) {
        var name;
        if (g.hasEdge(v.id, w.id) && g.isMultigraph()) {
          name = _.uniqueId("edge");
        }
        if (!g.hasEdge(v.id, w.id, name)) {
          g.setEdge(v.id, w.id, _.clone(_.last(defaultStack).edge), name);
        }
        if (v.port) { _.merge(g.edge(v.id, w.id, name), { $out: v.port }); }
        if (w.port) { _.merge(g.edge(v.id, w.id, name), { $in: w.port }); }
        _.merge(g.edge(v.id, w.id, name), attrs);
      });
    });
...
function collectNodes(stmt) {
  ...
  while(stack.length) {
    curr = stack.pop();
    switch(curr.type) {
      case "node": nodes.push(curr); break;
