...
function buildGraph(parseTree) {
  ...
  _.each(parseTree.stmts, stmt => handleStmt(g, stmt, defaultStack));
  return g;
}
...
function handleSubgraphStmt(g, stmt, defaultStack, sg) {
  ...
  defaultStack.push(Object.assign({}, _.last(defaultStack)));
...
function handleAttrStmt(g, stmt, defaultStack) {
  Object.assign(_.last(defaultStack)[stmt.attrType], stmt.attrs);
}
...
function maybeCreateNode(g, v, defaultStack, sg) {
  if (!g.hasNode(v)) {
    g.setNode(v, Object.assign({}, _.last(defaultStack).node));
    g.setParent(v, sg);
  }
}
