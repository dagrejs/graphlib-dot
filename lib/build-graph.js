    ...
    _.each(prev, function(v) {
      _.each(curr, function(w) {
        ...
        if (!g.hasEdge(v, w, name)) {
          g.setEdge(v, w, _.cloneDeep(_.last(defaultStack).edge), name);
        }
...
function handleSubgraphStmt(g, stmt, defaultStack, sg) {
  ...
  defaultStack.push(_.cloneDeep(_.last(defaultStack)));
...
function maybeCreateNode(g, v, defaultStack, sg) {
  if (!g.hasNode(v)) {
    g.setNode(v, _.cloneDeep(_.last(defaultStack).node));
    g.setParent(v, sg);
  }
}
