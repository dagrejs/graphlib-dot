...
function dotify(SuperConstructor) {
  function Constructor() {
    ...
    // Map of subgraph id (or null) -> type -> defaults. Note that we map DOT's
    // graph defaults to the "subgraph" type.
    this._defaults = { null: { node: {}, edge: {}, subgraph: {} } };
  }
  ...
  Constructor.prototype.defaults = function(id, type, value) {
    var defaults = this._defaults[id];
    if (defaults === undefined) {
      throw new Error("No subgraph with id " + id + " in this graph.");
    }

    var defaultsForType = defaults[type];
    if (defaultsForType === undefined) {
      throw new Error("Unrecognized type for defaults: " + type);
    }

    if (arguments.length < 3) {
      return defaultsForType;
    }
    this._checkValueType(value);
    defaults[type] = value;
  };
  ...
  Constructor.prototype.addSubgraph = function(id, value) {
    ...
    this._defaults[id] = { node: {}, edge: {}, subgraph: {} };
    return id;
  };
